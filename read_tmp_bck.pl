...
# If it is main() function.
	  ...
	  if($input =~/main\((.*),(.*)\)/)
	  {
		...
		if($f=~/\s*char\s+\**(.*)(\[\])*/)     ### IT's For character
		{
...
###  
  ...
  if($input=~/(\s*)memcpy(.*),(.*),(.*)/)
  {
   ...
   if($input=~/\s*([.A-Za-z0-9_]*(\->)*[.A-Za-z0-9_]*)\s*(=+)\s*(.*)\(*$variable(.*)\)*/)  # Tainted PAttern Found
    {
...
my $tmp_fun_name = $temp_ary[$fun_index];
 ...
 ########## Important Regular Expression which check for Function Call which is called by Tainted Data 
 ########## I
 
 if($input=~/(\s*)(=+)(\s*)$tmp_fun_name\(+(.*)\)+/)
 {
      # print $input."\n";		
#print $tmp_fun_name;
#	print $4;
	...
	$fun_taint_index++;	      
                             ...
                             while($ty_count < $start_of_function[$fun_index] and $line_function = <$t_temp>)
			     {
			       ...
			       if($line_function =~/\s*(int|float|char|struct|void)(\*|\s)+[a-z|A-Z_]+\((.*)\)\s*/)
			       {
#			         print $line_function;
	#			 print $3;
				 my $temp = $3;
			...
			my (@new_tmp_tainted_variables) = split ",",$tainted_variables[$fun_index];
			my $multi_flag =0;
			my $new__var = 0;
			
			foreach $new__var(@new_tmp_tainted_variables)
			{
			#print $new__var."\n";
			#print $temp."\n------------------\n";
			      if($temp eq $new__var)
			      {
			          $multi_flag = 1;
				  print $outputfile "$file_name $fun_name $counter $depth $tainted_variables[$fun_index]\n";
			      }
